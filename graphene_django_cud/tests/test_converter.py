...
class TestConvertChoicesField(TestCase):
    def test__field_with_choices__is_converted_to_enum(self):
        class MockChoicesModel(models.Model):
        ...
        field = MockChoicesModel._meta.get_field("field_with_choices")
...
class TestConvertDjangoFieldToInput(TestCase):
    def test__decimal_field__is_converted_to_graphene_decimal(self):
        class MockDecimalModel(models.Model):
        ...
        field = MockDecimalModel._meta.get_field("percent")
...
class ConvertDjangoFieldWithChoices(TestCase):
    def test__field_has_default__required_is_set_appropriately(self):
        class MockDefaultModel(models.Model):
            field_with_choices = models.CharField(
                max_length=16,
                choices=(
                    ("A", "Choice a"),
        ...
        field = MockDefaultModel._meta.get_field("field_with_choices")
    ...
    def test_choices_field__choice_field_in_different_locations__accepts_different_parameters(
        ...
        class MockRegistryModel(models.Model):
            field_with_choices = models.CharField(
                max_length=16,
                choices=(
                    ("A", "Choice a"),
        ...
        field = MockRegistryModel._meta.get_field("field_with_choices")
    ...
    def test__one_to_one_field__is_properly_converted(self):
        class MockOneToOneSuperModel(models.Model):
            name = models.CharField(max_length=128, default="Heidi Klum")

        class MockOneToOneModel(models.Model):
            super_model = models.OneToOneField(
                MockOneToOneSuperModel, on_delete=models.CASCADE, related_name="modelling"
            )  # Model-ling... Get it? xD

        # Convert primary relation
        registry = get_global_registry()
        field = MockOneToOneModel._meta.get_field("super_model")
        result = convert_django_field_with_choices(field, registry=registry)
        ...
        field = MockOneToOneSuperModel._meta.get_field("modelling")
        result = convert_django_field_with_choices(field, registry=registry)
